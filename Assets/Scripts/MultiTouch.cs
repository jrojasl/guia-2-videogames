    ...
    void Update()
    {
        ...
        if (Input.touchCount > 0)
        {
            for(int i = 0; i< Input.touchCount; i++)
            {
                ...
                string info = "Touch :" + i.ToString() + "-" + "Tap Count: " + theTouch.tapCount.ToString() + " - " + "Finger ID: " + theTouch.fingerId.ToString() + " - " + "nRadius" +
                    theTouch.radius.ToString() + " - " + ((theTouch.radius / (theTouch.radius + theTouch.radiusVariance)) * 100f).ToString() + "%"  + "\n";
